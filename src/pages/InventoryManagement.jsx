...
const InventoryManagement = ({ theme, setTheme }) => {
    ...
    try {
      if (selected.includes('all')) {
        ...
        for (const cat of selected) {
          await axios.delete(`/api/products/category/${encodeURIComponent(cat)}`);
        }
        // No success alert
      }
      fetchProducts();
  ...
  const updateStock = async (id, newStock) => {
    try {
      await axios.put(`/api/products/${id}/stock`, { stock: Number(newStock) });
  ...
  const handleExportMobiles = async () => {
    try {
      const response = await axios.get("/api/export/mobiles", { responseType: "blob" });
  ...
  const handleExportAccessories = async () => {
    try {
      const response = await axios.get("/api/export/accessories", { responseType: "blob" });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
  ...
  const handleImportExcel = async (event) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post("/api/import/products", formData, { headers: { "Content-Type": "multipart/form-data" } });
